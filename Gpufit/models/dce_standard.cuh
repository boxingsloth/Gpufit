#ifdef USE_DCE_STANDARD
#define GPUFIT_DCE_STANDARD_CUH_INCLUDED

__device__ REAL get_dce_standard_value (
	REAL p0, //Ktrans
	REAL p1, //Kep
	REAL p2, //Vp
...
__device__ void calculate_dce_standard (               // function name
	...
	std::size_t const user_info_size)
{
	...
	value[point_index] = get_dce_standard_value(parameters[0],parameters[1],parameters[2],point_index,T,Cp);


	/////////////////////////// derivative ///////////////////////////
	REAL * current_derivative = derivative + point_index;
	bool numericalApproximationDerivative = true;
	// don't set true as this is not yet implemented
	...
	if (numericalApproximationDerivative)
	{
		...
		// parameters[0]' = (Ktrans)'
		f_plus_h = get_dce_standard_value (parameters[0]+h,parameters[1],parameters[2],point_index,T,Cp);
		f_minus_h = get_dce_standard_value (parameters[0]-h,parameters[1],parameters[2],point_index,T,Cp);
		current_derivative[0 * n_points] = 1/(2*h)*(f_plus_h-f_minus_h);

		// parameters[1]' = (Kep)'
		f_plus_h = get_dce_standard_value (parameters[0],parameters[1]+h,parameters[2],point_index,T,Cp);
		f_minus_h = get_dce_standard_value (parameters[0],parameters[1]-h,parameters[2],point_index,T,Cp);
		current_derivative[1 * n_points] = 1/(2*h)*(f_plus_h-f_minus_h);

		// parameters[2]' = (Vp)'
		f_plus_h = get_dce_standard_value (parameters[0],parameters[1],parameters[2]+h,point_index,T,Cp);
		f_minus_h = get_dce_standard_value (parameters[0],parameters[1],parameters[2]-h,point_index,T,Cp);
